...
async function createIndexes() {
  ...
  try {
    ...
    console.log('\n📊 Quiz Submissions Collection...');
    await db.collection('quizSubmissions').createIndex({ quizId: 1, userId: 1 });
    await db.collection('quizSubmissions').createIndex({ quizId: 1, score: -1 });
    await db.collection('quizSubmissions').createIndex({ userId: 1, submittedAt: -1 });
    ...
    console.log('\n📊 Connection Requests Collection...');
    await db.collection('connectionRequests').createIndex({ from: 1, to: 1 }, { unique: true });
    ...
    console.log('\n📊 Resource Comments Collection...');
    await db.collection('resourceComments').createIndex({ resourceId: 1, createdAt: -1 });
    ...
    console.log('\n📊 Resource Ratings Collection...');
    await db.collection('resourceRatings').createIndex({ resourceId: 1, userId: 1 }, { unique: true });
    await db.collection('resourceRatings').createIndex({ resourceId: 1, rating: -1 });

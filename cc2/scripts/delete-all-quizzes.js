...
async function deleteAllQuizzes() {
  ...
  try {
    ...
    const submissionsResult = await db.collection('quizSubmissions').deleteMany({});
    console.log(`✅ Deleted ${submissionsResult.deletedCount} quiz submissions`);
    
    // Delete quiz randomizations
    console.log('\nDeleting quiz randomizations...');
    const randomizationsResult = await db.collection('quizRandomizations').deleteMany({});
    console.log(`✅ Deleted ${randomizationsResult.deletedCount} quiz randomizations`);

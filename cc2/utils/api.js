...
export async function updateResourceDislikes(resourceId) {
  try {
    const response = await fetch("/api/resources/dislike", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(errorData.error || "Failed to update dislikes");
    }

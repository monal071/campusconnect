...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get post
    const post = await db.collection('communityPosts').findOne({ _id: new ObjectId(postId) });
    ...
    if (!isCreator && !isAuthor) {
      return res.status(403).json({ message: 'Only community creator or post author can delete posts' });
    }

    // Delete post
    await db.collection('communityPosts').deleteOne({ _id: new ObjectId(postId) });

...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const collections = await db.collection('resourceCollections')
        .find(query)
        .sort({ createdAt: -1 })
        .toArray();
      ...
      const collectionsWithDetails = await Promise.all(
        collections.map(async (collection) => {
          const resourceIds = collection.resources?.map(r => r.resourceId) || [];
    ...
    if (req.method === 'POST') {
      ...
      const { name, description, resources = [], tags = [], isPublic = false } = req.body;
      ...
      const result = await db.collection('resourceCollections').insertOne(newCollection);
    ...
    if (req.method === 'PUT') {
      // Update collection
      const { collectionId, name, description, resources, tags, isPublic } = req.body;

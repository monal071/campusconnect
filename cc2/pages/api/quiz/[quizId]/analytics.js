...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (quiz.userId.toString() !== session.user.id && session.user.role !== 'admin') {
      ...
      if (range !== 'all') {
        ...
        filteredSubmissions = submissions.filter(s => 
          new Date(s.submittedAt) >= rangeDate
        );
      }
      ...
      const averageScore = totalAttempts > 0
        ? Math.round(filteredSubmissions.reduce((sum, s) => sum + (s.score || 0), 0) / totalAttempts)
        : 0;

      // Pass rate (assuming 60% is passing)
      const passThreshold = 60;
      const passedCount = filteredSubmissions.filter(s => (s.score || 0) >= passThreshold).length;
      ...
      const averageTime = totalAttempts > 0
        ? filteredSubmissions.reduce((sum, s) => {
            ...
            if (s.submittedAt && s.startedAt) {
              return sum + Math.floor((new Date(s.submittedAt) - new Date(s.startedAt)) / 1000 / 60);
            }
            return sum;
          }, 0) / totalAttempts
        : 0;
      ...
      const questionStats = quiz.questions.map((question, index) => {
        ...
        const correctResponses = questionResponses.filter(a => 
        ...
        const skipped = filteredSubmissions.filter(s => 
          s.answers?.[index] === undefined || s.answers?.[index] === null
        ).length;
      ...
      const topPerformers = filteredSubmissions
        ...
        .map(s => ({
          ...
          score: s.score || 0,
          time: s.timeSpent || Math.floor((new Date(s.submittedAt) - new Date(s.startedAt)) / 1000 / 60),
        }));
      ...
      if (range !== 'all') {
        ...
        if (previousSubmissions.length > 0) {
          const prevAvgScore = Math.round(
            previousSubmissions.reduce((sum, s) => sum + (s.score || 0), 0) / previousSubmissions.length
          );
          const prevPassRate = Math.round(
            (previousSubmissions.filter(s => (s.score || 0) >= passThreshold).length / previousSubmissions.length) * 100
          );

...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { quizId, answers, studentId, studentName, timeSpent } = req.body;
    ...
    const averageScore =
      totalSubmissions > 0
        ? allSubmissions.reduce((sum, sub) => sum + sub.percentageScore, 0) /
          totalSubmissions
        : 0;
    ...
    const passRate =
      totalSubmissions > 0
        ? (allSubmissions.filter((sub) => sub.percentageScore >= 60).length /
            totalSubmissions) *
          100
        : 0;

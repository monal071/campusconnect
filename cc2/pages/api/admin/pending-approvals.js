  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "approve") {
        ...
        // Invalidate events cache so newly approved events show up immediately
        if (
          type === "event" &&
          invalidateCache &&
          typeof invalidateCache.events === "function"
        ) {
          try {
            await invalidateCache.events();
          } catch (err) {
            console.error("Error invalidating events cache:", err);
          }
        }

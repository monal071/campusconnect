        ...
        bookmarks.map(async (bookmark) => {
          ...
          try {
            switch (bookmark.type) {
              case 'post':
                item = await db.collection('posts').findOne({ _id: new ObjectId(bookmark.itemId) });
                break;
              case 'resource':
                item = await db.collection('resources').findOne({ _id: new ObjectId(bookmark.itemId) });
                break;
              case 'event':
                item = await db.collection('events').findOne({ _id: new ObjectId(bookmark.itemId) });
                break;
              case 'job':
                item = await db.collection('jobs').findOne({ _id: new ObjectId(bookmark.itemId) });
                break;
              case 'quiz':
                item = await db.collection('quizzes').findOne({ _id: new ObjectId(bookmark.itemId) });
                break;
            }
          } catch (error) {
